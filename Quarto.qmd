...
Pour installer Quarto, il suffit de visiter le site de [Quarto](https://quarto.org/) de la compagnie Posit. Cliquez sur `Get Started` et ensuite sur `Download Quarto CLI`. Par défaut, Quarto détecte votre système d'exploitation (Ubuntu, Linus, Mac, Windows). Une fois le fichier téléchargé, il faut l'installer comme les autres logiciels.
...
```{r}
#| label: fig-quartoversion
#| fig-cap: "Vérification de la version intallée de Quarto"
#| fig-align: left
#| out.width: "100%"
#| echo: false
knitr::include_graphics('figures/quarto_version.png')
```
