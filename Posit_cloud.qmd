...
Dans ce chapitre, j'introduis l'application [RStudio Cloud](https://www.rstudio.com/products/cloud/). RStudio Cloud est la version web de RStudio IDE. Elle permet : 
...
Bien que RStudio Cloud s'apparente à la version locale[^locale] de RStudio IDE, les premiers pas avec RStudio Cloud peuvent s'avérer difficiles. Dans le présent chapitre, je désire soulever quelques éléments importants afin de maximiser l'expérience d'utilisation de RStudio Cloud.

[^locale]: L'expression `version locale` désigne la version de RStudio téléchargée et installée sur un ordinateur (et non dans le nuage, comme RStudio Cloud).

## Création d'un compte

La première étape consiste à créer un compte gratuit (*Free*) sur le site web de [RStudio Cloud](https://www.rstudio.com/products/cloud/) (voir Figure \@ref(fig:signup)). Notez, par ailleurs, que l'utilisateur atteint très rapidement les limites d'utilisation du plan gratuit de RStudio Cloud. Dans l'optique où l'utilisateur (i) dépasse le nombre de projets ou d'espaces de travail admissibles, (ii) dépasse le nombre d'heures par projet disponibles, ou (iii) qu'il désire augmenter la mémoire RAM et la vitesse d'exécution des codes, le plan *Premium* de RStudio Cloud s'avère une alternative intéressante, mais relativement dispendieuse[^premium].

[^premium]: Cela est d'autant plus vrai que la version locale de RStudio IDE est, elle, disponible gratuitement.

```{r signup, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Procédure de  création d'un compte gratuit avec RStudio Cloud"}
...
```{r ws, echo=FALSE, out.width='40%', fig.align='center', fig.cap="Espace de travail individuel de RStudio Cloud"}
...
[^class]: RStudio Cloud a d'ailleurs été conçu, en partie, pour répondre à ce besoin lors de l'enseignement de R et de RStudio.
...
```{r spaces, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Illustration de la fenêtre d'ouverture de RStudio Cloud"}
...
[^projetR]: Si vous n'êtes pas familier avec les projets RStudio, je vous invite à consulter le chapitre \@ref(projetRStudio).

```{r newproject, echo=FALSE, out.width='50%', fig.align='center', fig.cap="Création d'un nouveau projet dans RStudio Cloud"}
...
```{r untitled, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Untitled project dans RStudio Cloud"}
...
```{r exercice2, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Modifier le nom d'un nouveau projet dans RStudio Cloud"}
...
3. Après l'importation, le fichier (`RatLives.csv`) est disponible dans le projet RStudio Cloud.
...
```{r upload, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Importer un fichier dans un projet RStudio Cloud"}
...
1. Je peux cliquer sur l'onglet `Exercices [Sylvain Fiset]` dans la liste des espaces à la gauche de la fenêtre de RStudio Cloud (sous l'en-tête `Spaces`).
...
```{r liste, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Examiner la liste de tous les projets d'un espace de travail dans RStudio Cloud"}
knitr::include_graphics('figures/RScloud-ProjectsList.png')
```

### Exporter un projet RStudio Cloud
...
[^share]: Si vous ne désirez pas utiliser la fonction `share` de RStudio Cloud.
...
```{r export, echo=FALSE, out.width='50%', fig.align='center', fig.cap="Exportation d'un projet à partir de RStudio Cloud"}
knitr::include_graphics('figures/RScloud-export.png')
```

### Partager un projet dans RStudio Cloud
...
```{r members, echo=FALSE, out.width='100%', fig.align='center', fig.cap="Ajouter des membres à un espace de travail RStudio Cloud"}
...
```{r access, echo=FALSE, out.width='50%', fig.align='center', fig.cap="Contrôler l'accessibilité à un projet RStudio Cloud"}
...
```{r delete, echo=FALSE, out.width='50%', fig.align='center', fig.cap="Effacer un projet à partir de RStudio Cloud"}
knitr::include_graphics('figures/RScloud-deleteProject.png')
```

## Options de RStudio Cloud
...
```{r account, echo=FALSE, out.width='75%', fig.align='center', fig.cap="Paramètres du compte de l'utilisateur"}
knitr::include_graphics('figures/RScloud-account.png')
```

## Quitter RStudio Cloud
