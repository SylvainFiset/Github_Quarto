...
Bien que Posit Cloud s'apparente à la version locale[^posit_cloud-1] de RStudio IDE, les premiers pas avec Posit Cloud peuvent s'avérer difficiles. Dans le présent chapitre, je désire soulever quelques éléments importants afin de maximiser l'expérience d'utilisation de Posit Cloud.
...
Dans le cadre d'un cours universitaire ou collégial, je recommande fortement l'utilisation d'un compte *Cloud Instructor*. Ce compte permet au professor d'héberger tous les projets de ses étudiantes et étudiants dans un espace de travail (`spaces`) dédié à un cours. Le coût --- pour le professeur et les étudiante et étudiants --- est faible et facilite grandement le travail du professeur.

Le partage de l'espace de travail peut se faire via une invitation individuelle ou via un lien de partage. De plus, le professeur peut créer des *gabarits* (templates) qui sont copiés par les étudiantes et étudiants. Ces gabarits peuvent inclure, entre autres, une série d'extension déjà téléchargées dans l'espace de travail et des scripts qui seront modifiés par les étudiantes et étudiants.
...
Néanmoins, dans le cadre d'un cours, le professeur peut créer un gabarit qui sert de base pour la construction d'un exercice particulier, incluant les extensions R nécessaires à sa réalisation. Dans ce cas là, l'utilisation du gabarit est recommandée. Si cela se produit, le gabarit sera répliqué dans votre compte sur Posit Cloud et vous pourrez le modifier à volonté comme si c'était votre projet.

### Devoirs (`Assignments`)

Dans le cadre d'un cours, un professeur peut créer un projet RStudio qui contient tous les fichiers (données, explications, solution) nécessaires à la réalisation d'un exercice --- devoir --- spécifique. Si le professeur produit un devoir (`assignment`) dans Posit Cloud, vous recevrez un lien par courriel (ou autre moyen de communication électronique utilisé par le professeur). Il suffit de cliquer sur le lien et d'ouvrir votre compte Posit Cloud. Vous verrez un projet avec un icone bleu du nom de `Start`. Cliquez sur cette icone pour dupliquer le projet du professeur. Par la suite, je suggère de changer le nom de ce projet dupliqué pour y ajouter votre nom. Le professeur pouvoir avoir accès à votre projet RStudio directement dans son propre compte.

Une fois que le projet du devoir fut dupliqué et renommé, il demeure accessible dans votre fenêtre `All Content`. Mais, ne l'utilisez pas. Il faut maintenant travailler avec le projet auquel vous avez ajouté votre nom. C'est ce projet que le professeur voit dans son espace de travail. 

La @fig-duplique montre que le projet avec votre nom (étudiant) à utiliser est présent dans votre espace de travail. Vous pouvez le modifier à volonter puisqu'il vous appartient. Remarquez tous les petits icones à la droite de ce projet renommé, ce qui n'est pas le cas pour le projet partagé par le professeur.
