...
Par ailleurs, dans le cadre d'un cours, il est fort possible que l'université exige l'utilisation de Posit Cloud. Cela facile grandement le travail de la professeure ou du professeur puisque tous les projets sont regroupés dans le compte du professeur. Dans ce cas, veuillez suivra la démarche de création d'un compte Posit Cloud (voir la @sec-comptegratuit).
...
![Get started](figures/posit_cloud_getstarted.png){#fig-getstarted}
...
Dans la page d'ouverture de l'espace de travail partagé avec votre professeur, vous verrez un projet avec un icone bleu du nom de `Start` (voir la @fig-start). Cliquez sur cette icone pour dupliquer le projet du professeur. Par la suite, je suggère de changer le nom de ce projet dupliqué pour y ajouter votre nom. Le professeur pouvoir avoir accès à votre projet RStudio directement dans son propre compte.

![Icone Start pour un nouveau devoir](figures/posit_cloud_start.png){#fig-start}

Une fois que le projet du devoir fut dupliqué, je suggère de changer le nom de ce projet dupliqué pour y ajouter votre nom. Le professeur pouvoir avoir accès à votre projet RStudio directement dans son propre compte. De plus, le projet partagé avec le professeur demeure accessible dans votre fenêtre `All Content` avec un icone du nom de `Continue`. Mais, ne l'utilisez pas. Il faut maintenant travailler avec le projet auquel vous avez ajouté votre nom. C'est ce projet que le professeur voit dans son espace de travail.

![Icone Continue](figures/posit_cloud_assignment_continue.png)

La figure ... montre que le projet avec votre nom (étudiant) à utiliser est présent dans votre espace de travail. Vous pouvez le modifier à volonter puisqu'il vous appartient. Remarquez tous les petits icones à la droite de ce projet renommé, ce qui n'est pas le cas pour le projet partagé par le professeur.
